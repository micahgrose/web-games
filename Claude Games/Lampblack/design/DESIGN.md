...
**Chase & capture:** DETECTED → guard chases at sprint-equal speed but loses you on broken LOS + 3s (→ ghost search). While chased, your sprint is free (stealth already blown locally). **Grabbed** (guard adjacent 0.5s): if you hold a **Last Trick** (start with 1; Bruiser 2; smoke tier-3 refunds one) it auto-burns — soot-cloud burst, guard blinded, you're dropped 3 tiles away, +10 Alertness. **No Trick = PINCHED = run over** (keep only rep already fenced). You can always flee to any unsealed exit with whatever's in the bag — the mercy is retreat, never rescue. *Rejected:* HP bar (this is not a combat game; capture-as-death keeps every guard lethal and every decision about information, not attrition); mid-run jail-break minigame (cool, noted for post-ship content, but it softens the ratchet's teeth for v1).
...
- **What a fresh me might miss reading this:** the three channels are ONE fairness contract — every rule that watches the player is rendered (cones, fills, rings, ghosts, footprints). If any Phase 4 shortcut breaks render-what-you-simulate, the knife-edge risk comes back. Non-negotiable.

## Open questions
- ~~Phase 1: user pick~~ ANSWERED: **Lampblack**, no splice, directive "watch art + sounds" (standing, in HANDOFF).
- Phase 3: teaching curriculum beats; full feel spec (frames, synthesis recipes, palette, layout).
- Phase 4: fire propagation stretch (in/out); safecracking minigame tuning (user co-tune); mid-run save shape; exact numbers pass.
- For the USER at this seam (your "poke" window): does gaslamp-noir Victorian land for you, or want a re-flavor (same systems wear many skins — 1920s, fantasy, oriental-express)? Any tool/guard/Score you want added to the rosters? Is "pinched = run over, one Last Trick" the right harshness?
